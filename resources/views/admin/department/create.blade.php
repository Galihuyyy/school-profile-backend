<x-layouts.admin-layout>
    <x-slot name="title">Tambah Jurusan</x-slot>
    <x-slot name="subtitle">Tambahkan data jurusan baru ke sistem.</x-slot>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm">
        <div class="p-8 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-extrabold text-slate-900">Form Tambah Jurusan</h3>
                    <p class="text-xs text-slate-400">Isi semua field yang wajib diisi</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin::departments.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
