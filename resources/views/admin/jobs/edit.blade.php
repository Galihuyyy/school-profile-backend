<x-layouts.admin-layout>
    <x-slot name="title">Edit Lowongan</x-slot>
    <x-slot name="subtitle">Form Edit Lowongan <b>{{ $job->company_name }} - {{ $job->company_name }}</b></x-slot>

    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm" x-data="jobForm()" x-init="init()" x-cloak>
        <div class="p-8 border-b border-slate-100">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin::jobs.index') }}" class="w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition-colors">
        ...
        <form action="{{ route('admin::jobs.update', $job) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-8 border-t border-slate-100">
                <a href="{{ route('admin::jobs.index') }}" class="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-sm font-bold transition-colors">
                    Batal
                </a>
