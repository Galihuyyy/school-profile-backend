        ...
        <form action="{{ route('admin::jobs.store') }}" method="POST" class="p-8">
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-8 border-t border-slate-100">
                ...
                @if (hasPermission('manage_job'))
                    <a href="{{ route('admin::jobs.edit', $job) }}" class="px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-sm font-bold shadow-sm shadow-teal-600/20 transition-colors">
                        Edit Data
                    </a>
                @endif
            </div>
