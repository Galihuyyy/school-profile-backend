<x-layouts.admin-layout>
    ...
    <x-slot name="subtitle">Perbarui data guru {{ $teacher->name }}.</x-slot>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm">
        <div class="p-8 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-extrabold text-slate-900">Form Edit Guru</h3>
                    <p class="text-xs text-slate-400">Kosongkan field foto jika tidak ingin mengubah foto</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin::teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            @method('PUT')
